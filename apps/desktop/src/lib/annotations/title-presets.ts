...
import type { AnnotationGlow, AnnotationKind } from "$lib/stores/editor-store.svelte";
...
function text(over: Partial<TextKind> & Pick<TextKind, "x" | "y" | "w" | "h" | "content">): TextKind {
	return {
		kind: "text",
		fontFamily: FONT,
		fontSize: 0.06,
...
export const TITLE_PRESETS: TitlePreset[] = [
	...
	},
	{
		...
		glow: LEGIBILITY_GLOW,
		build: () =>
			text({ x: 0.18, y: 0.3, w: 0.64, h: 0.1, content: "Subtitle", fontSize: 0.045, fontWeight: 500 }),
	},
	{
		id: "lower-third",
		label: "Lower third",
