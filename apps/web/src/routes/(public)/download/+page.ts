...
export const load: PageLoad = async ({ fetch, setHeaders }) => {
	...
	try {
		const response = await fetch(
			"https://api.github.com/repos/kanakkholwal/recast/releases/latest",
		);
		...
		const macosIntel = assets.find(
			(a) => a.name.includes("x64") && a.name.endsWith(".dmg") && !a.name.endsWith(".sig"),
		);
		const macosAppleSilicon = assets.find(
			(a) => a.name.includes("aarch64") && a.name.endsWith(".dmg") && !a.name.endsWith(".sig"),
		);

		// Linux
		const linuxAppImage = assets.find(
