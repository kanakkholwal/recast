...
{#if ready}
	...
	>
		...
		<!-- Corner labels -->
		<span
			class="pointer-events-none absolute left-3 top-3 rounded-full bg-black/55 px-2.5 py-1 text-caption font-semibold text-white/85 ring-1 ring-inset ring-white/15 backdrop-blur"
		>
			{raw.label}
		</span>
		<span
			class="pointer-events-none absolute right-3 top-3 rounded-full bg-primary/20 px-2.5 py-1 text-caption font-semibold text-primary ring-1 ring-inset ring-primary/30 backdrop-blur"
		>
			{polished.label}
		</span>
		...
		{#if applied.length > 0}
			...
			>
				{#each applied as feat (feat)}
					<span
						class="inline-flex items-center gap-1 rounded-full bg-black/55 px-2 py-0.5 text-caption font-semibold text-white/90 ring-1 ring-inset ring-white/15 backdrop-blur"
					>
		...
		>
			...
			></div>
			<div
				class="absolute left-1/2 top-1/2 grid size-10 -translate-x-1/2 -translate-y-1/2 place-items-center rounded-full bg-white text-black ring-1 ring-black/10 transition-transform group-active:scale-95"
			>
...
{:else}
	...
	<div
		class="relative grid aspect-video w-full place-items-center overflow-hidden rounded-2xl border border-border-low bg-background"
	>
		<div
			aria-hidden="true"
		...
		<div class="relative flex flex-col items-center gap-3 text-center">
			<span
				class="grid size-14 place-items-center rounded-2xl border border-border-low bg-paper text-muted-foreground backdrop-blur-sm"
			>
				<Film class="size-6" />
			</span>
			<div
