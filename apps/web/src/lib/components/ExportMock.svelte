...
const progress = $derived(reduced ? 100 : Math.min(100, Math.round((tick / UPLOAD_TICKS) * 100)));
</script>

<div
	class="rounded-xl border border-border-low bg-background/80 p-4 shadow-craft-inset"
>
	<div class="flex items-start gap-3">
		...
		<div class="min-w-0 flex-1">
			<div class="text-body-sm font-semibold tracking-tight text-foreground">
				Export complete
			</div>
	...
	<div class="mt-3 rounded-lg border border-border-low bg-paper px-3 py-2">
		{#if done}
			<div class="flex items-center gap-2">
				<HardDriveUpload class="size-3.5 shrink-0 text-success" />
				<span class="text-caption font-medium text-foreground">Uploaded to Drive</span>
				<span
					class="ml-auto inline-flex items-center gap-1 rounded-md border border-border-low bg-background px-1.5 py-0.5 text-caption font-semibold text-foreground"
				>
		...
		{:else}
			<div class="flex items-center gap-2">
				...
				<span class="text-caption font-medium text-foreground">Uploading to Drive…</span>
				<span class="ml-auto font-mono text-caption font-semibold text-muted-foreground">
					{progress}%
				</span>
			</div>
