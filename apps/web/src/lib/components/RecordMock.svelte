...
<div class="p-5">
	<div
		class="relative rounded-xl border border-border-low bg-background/60 p-4 shadow-craft-inset"
	>
		<div
			class="flex items-center gap-3 rounded-lg border border-border-low bg-background/80 px-3 py-2.5"
		>
			<Search class="size-4 text-muted-foreground" />
			<span class="text-sm font-medium text-foreground">Start a recording…</span>
			<span
				class="ml-auto rounded-md border border-border-low bg-background px-1.5 py-0.5 font-mono text-caption font-semibold text-muted-foreground"
			>
				⌘ ⇧ R
			</span>
		</div>
			...
			{#each options as opt, i (opt.label)}
				...
				<div
					class={cn(
						...
						i === active ? "bg-paper text-foreground" : "text-muted-foreground",
					)}
				>
					...
					{#if i === active}
						<span
							class="ml-auto rounded border border-border-low bg-background px-1 py-0.5 font-mono text-caption text-muted-foreground"
						>
							↵
						</span>
					{/if}
