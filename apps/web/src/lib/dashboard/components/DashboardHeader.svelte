...
<!-- Sticky inside the inset panel: on md+ the panel is inset by `m-2`, so the
	 ...
	 corners. -->
<header
	class="sticky top-0 z-20 flex h-14 shrink-0 items-center gap-2.5 border-b border-border-low bg-background px-4 md:top-2 md:rounded-t-xl"
>
	<Sidebar.Trigger
		class="size-7 shrink-0 rounded-md text-muted-foreground transition-colors hover:bg-paper hover:text-foreground"
		title="Toggle sidebar (⌘B)"
	/>
	<span class="h-5 w-px shrink-0 bg-border-low"></span>
	<nav class="hidden shrink-0 items-center gap-1.5 text-body-sm sm:flex" aria-label="Breadcrumb">
		<span class="font-medium text-foreground">{crumb.section}</span>
		{#if crumb.sub}
