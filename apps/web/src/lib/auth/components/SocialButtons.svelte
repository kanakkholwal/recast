<script lang="ts">
	...
	// Social sign-in is disabled in production until OAuth apps are configured.
	// Local dev keeps the buttons so we can iterate on the UI.
	...
	async function signInWith(provider: SocialProvider) {
		...
		if (error) toast.error(error.message ?? `Couldn't sign in with ${provider}.`);
	}
</script>
