<script lang="ts">
	...
	import { externalAttrs, resolveMenuWidths } from "./FloatingMenu.logic";
	import type { Props } from "./FloatingMenu.types";
	...
	$effect(() => {
		...
		const init = async () => {
			...
			const { isMobile, maxWidthOpen, maxWidthInitial } = resolveMenuWidths(
				window.innerWidth,
			);
