...
- **Picture time is a free-running clock, not `<video>.currentTime`.** A
  ...
  and asks the decoder for the matching frame (`clock.ts:1`,
  `VideoPreview.svelte:189`).
...
| --- | --- | --- |
| `VideoPreview.svelte` | `components/VideoPreview.svelte:1` | Owns the on-screen canvas, the rAF draw loop, the picture clock, AV-sync, MediaBunny source lifecycle, and the `<video>` fallback. |
| `draw()` | `VideoPreview.svelte:657` | Per-frame: derive `playbackTime`, pick/upload the frame texture, build `FrameInput`, call `RenderCore`. |
| `RenderCore` | `components/render-core.ts:39` | The one frame entry point: `computeFrameParams` → main pass → ordered overlay passes. Drives preview + export. |
| `WebGL2Backend` | `components/webgl2-backend.ts:58` | Owns the single compositor program + full-screen quad; `renderMain`, texture uploads, `drawSprite`. |
| `computeFrameParams` | `components/frame-params.ts:157` | Pure `(scene, geom, time) → FrameUniforms + svgCursor`. Mirrors the GLSL and the Rust export twin 1:1. |
| Compositor shader | `components/video-preview.shaders.ts:17` | Fragment shader: gradient, rounded-rect mask, zoom, motion blur, dot cursor, click highlight, drop shadow. |
| `OverlayQuad` | `components/overlay-quad.ts:73` | Textured-quad blitter (straight-alpha, rounded/circle mask) for export overlay passes. |
| `FrameTextureRing` | `lib/playback/frame-textures.ts:44` | Ring of GPU textures holding decoded frames; `bind(tUs, floorUs)` picks the newest in-segment frame. |
| `PlaybackClock` | `lib/playback/clock.ts:24` | Wall-clock integrator over gapless output time; the picture master on the MediaBunny path. |
...
(`VideoPreview.svelte:932-952`), and `stopVideoFrameLoop` paints once on the
...
(`VideoPreview.svelte:882-901`, `frame-textures.ts:30`). If no in-segment frame
is ready yet, `bindLast()` holds the last displayed frame.

**Play/pause effect** (`VideoPreview.svelte:1392-1423`). On the paused→playing
...
decoder's output surfaces (`VideoPreview.svelte:1347-1359`).

**How export reuses RenderCore** (`offscreen-export.ts:217`). Export builds a
...
- **Picture clock is master, not `<video>`.** On the MediaBunny path the
  ...
  the clock reintroduces the cut-freeze bug (`clock.ts:1`,
  `VideoPreview.svelte:667-701`).
- **`preserveDrawingBuffer: false` on the on-screen canvas** (`VideoPreview.svelte:287`).
  ...
  **in the same JS task** as `draw()`; an out-of-task `drawImage` samples a
  cleared buffer (`VideoPreview.svelte:553-584`, `1001-1042`). Export uses
...
- **The on-screen canvas is never `transferControlToOffscreen`-d.** The Phase-3
  ...
  it is deliberately avoided (`render-worker-client.ts:1-10`, `49`,
  `render-worker.ts:44-63`).
- **Zoom/cursor evaluator sync.** The cursor UV, click highlight, and SVG-cursor
  ...
  click point (`frame-params.ts:273-291`, `shaders:167,199-224`). Zoom lives in
  ...
  the Rust export rasteriser 1:1 (`frame-params.ts:1-11`, `shaders:1-6`).
- **Dot cursor is in the shader; every other cursor style is an overlay.**
  ...
  `svgCursor` instead (`frame-params.ts:282-307,346`); preview draws it as a DOM
...
- **Frame ring floor.** `bind(tUs, floorUs)` must reject frames before the
  ...
  cut (`frame-textures.ts:30-42`). Textures use `texStorage2D` (immutable) +
  `texSubImage2D` to avoid reallocating storage per 4K frame (`frame-textures.ts:108-128`).
- **MediaBunny↔`<video>` fallback is automatic and one-way per source.** A
  ...
  (`VideoPreview.svelte:882-901`, `1197-1332`). Context loss makes every GL call
  a silent no-op, so the loss event is `preventDefault`-ed to request
  restoration, and the dead ring is dropped (`VideoPreview.svelte:1049-1089`).
