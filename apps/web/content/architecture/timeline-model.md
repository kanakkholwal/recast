...
| --- | --- | --- |
| `TimelineCut`, `normalizeCuts` | `cuts.ts:13,26` | Removed original range; sort + merge overlapping/touching cuts (EPS 1e-4). |
| `originalToOutput` / `outputToOriginal` (cuts) | `cuts.ts:57,73` | Translation-only map: subtract/add removed spans. |
...
| `displayTimeMap` | `time-map.ts:108` | Transient full-source axis WHILE trimming (head/tail at 1×). |
| `buildGapMap` | `time-map.ts:140` | Re-spaces cuts as visible gaps for the "show cut gaps" view (render-only). |
| `originalToOutput` / `outputToOriginal` (map) | `time-map.ts:161,186` | Speed-aware piecewise map; binary-searched forward direction. |
| `planStoryboard`, `storyboardCrop`, `storyboardCoverCrop` | `storyboard.ts:22,67,94` | Hover-sprite grid geometry + CSS crop math. |
| `planFilmstrip`, `LruCache` | `filmstrip.ts:73,117` | Virtualized output-axis tile layout; bounded LRU with `onEvict`. |
| `createTileProvider` / `MediabunnyTileProvider` | `filmstrip-source.ts:315,76` | Main-thread tile/hover/storyboard provider; returns null → strip fallback. |
| `startFilmstripWorker`, `drain`, `buildStoryboard` | `filmstrip-worker.ts:241,133,174` | Off-thread decode; single-decoder drain latch. |
| Rust mirror: `collect_export_cuts`, `build_speed_segments`, `warped_output_duration`, `build_speed_setpts_expr`, `build_speed_audio_filter` | `cuts_speed.rs:23,110,159,188,236` | Export-side re-implementation of the identical math. |
...
1. Build the map once from the edit state:
   ...
   (`time-map.ts:71`, `segments.ts:43`, `segment-speed.ts:88`). Each kept span
   gets `outStart/outEnd` laid end-to-end, width `(origEnd-origStart)/speed`
   (`time-map.ts:61-66`).
2. `outputToOriginal(map, t)` (`time-map.ts:186`) walks spans: clamps t into the
...
- **Preview** routes playhead/scrub/waveform/filmstrip through the map above. The
  ...
  `playbackRate` per segment (`segment-speed.ts:42-55`).
- **Export** (`cuts_speed.rs`) rebuilds the same kept-segment + speed structure on
  the post-trim axis: `collect_export_cuts` (clamp/sort/merge, `:23`) →
...
2. `MediabunnyTileProvider` (`filmstrip-source.ts:76`) batches requests once per
   ...
   compete with the preview decoder for hardware slots (`setDecodePaused`, `:200`),
...
4. Hover-scrub uses either a decoded frame (`previewAt`, own 64-entry cache) or,
   ...
   `storyboardCoverCrop` (`storyboard.ts:67,94`).
5. **Fallback**: `createTileProvider` returns `null` (never throws) when
   ...
   Rust-generated strip (`filmstrip-source.ts:315-338`).
...
- **Cut collapse semantics.** Removed original time has no output image and maps
  onto the **seam** = the next kept span's `outStart` (`time-map.ts:177`,
  `cuts.ts:57`). Zero width on the output axis; rendered as a single marker at
  `gapStart` (`segments.ts:81`). Adjacent segments that merely *touch* (a split,
  nothing removed) produce no seam (`deriveSeams`, `segments.ts:99`).
...
- **`buildGapMap` is render-only.** It re-spaces cuts as visible gaps for the
  opt-in "show cut gaps" view (`time-map.ts:140`). Playback and export never use
...
- **Boundary EPS = 1e-4 on BOTH sides.** The tolerance for treating two times as
  ...
  (`cuts.ts:31`, `time-map.ts:19`, `segments.ts:14`, `segment-speed.ts:15`) and
  `CUT_MERGE_EPS = 1e-4` in Rust (`cuts_speed.rs:21`). They MUST stay in lockstep
...
- **TS↔Rust parity fixtures.** `__fixtures__/cut-parity.json` and
  ...
  - TS: `time-map.test.ts:160` / `cuts.test.ts` load `cut-parity.json`;
    `segment-speed.test.ts:108` loads `speed-parity.json`
    (`import … from "./__fixtures__/…json"`).
  - Rust: `cuts_speed.rs:526,563` load the SAME files via
  ...
  - **Assertion tolerance is tighter than the model EPS**: TS uses
    `toBeCloseTo(…, 6)` and Rust uses `abs() < 1e-6` (`cuts_speed.rs:551,604`).
...
- **Single decoder latch.** The filmstrip worker's `draining` boolean is
  ...
  (`filmstrip-worker.ts:127-133`). Every worker reply path (`tile`/`error`/`drop`)
  must release the request id or the tile wedges in-flight and the id maps grow
  unbounded (`filmstrip-source.ts:227-234`).
