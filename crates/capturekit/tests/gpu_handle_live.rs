...
/// Open the shared frame on `device`, wait for the fence, and read it back.
///
/// This is the consumer an encoder would be. Waiting is not optional: the
/// sharing carries no implicit barrier.
fn read_shared(
    device: &ID3D11Device,
    ...
    unsafe {
        ...
        device5
            .OpenSharedFence(HANDLE(handle.fence as *mut core::ffi::c_void), &mut fence)
            .expect("the shared fence opens");
        let context4: ID3D11DeviceContext4 = context.cast().expect("ID3D11DeviceContext4");
        context4
            .Wait(&fence.expect("a fence"), handle.ready_at)
