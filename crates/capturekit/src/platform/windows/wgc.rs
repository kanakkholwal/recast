...
/// Whether this build of Windows has Graphics Capture (10 2004+ and 11). Probed once, since the answer cannot change while the process runs.
/// `IsSupported` is a WinRT activation whose factory windows-rs caches globally; asked after that apartment is torn down the stale pointer faults, crashing take two.
