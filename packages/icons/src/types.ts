 ...
 *
 * Loose by design so Lucide's call shape (`class` + `size` + `strokeWidth`)
...
/**
 * Anything we can drop into a slot where today a Lucide icon goes.
 *
 ...
 * from the Lucide era (where icons are stored in data tables as values),
 * we widen the type to `any` for the storage-side, so a Lucide call like
