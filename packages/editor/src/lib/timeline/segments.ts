...
export function deriveSegments(shape: ClipShape): Segment[] {
	...
	const cuts = normalizeCuts(shape.cuts).filter((c) => c.end > trimStart && c.start < trimEnd);
 ...
 */
export function planDeleteSegment(seg: Segment, splitPoints: number[]): DeletePlan {
	return {
		cut: { start: seg.start, end: seg.end },
		splitPoints: splitPoints.filter((p) => p < seg.start - EPS || p > seg.end + EPS),
	};
}
