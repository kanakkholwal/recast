...
export function planStoryboard(durationSec: number): StoryboardPlan {
	const count = Math.min(MAX_CELLS, Math.max(MIN_CELLS, Math.ceil(Math.max(0, durationSec))));
...
export function storyboardSampleSec(cell: number, count: number, durationSec: number): number {
	if (count <= 0) return 0;
