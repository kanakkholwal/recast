...
import type { CursorStyleId } from "../../stores/editor-store.svelte";
...
export function cursorStyleSvg(style: CursorStyle, state: CursorStyleState): string {
	switch (state) {
		case "rightPress":
...
export function cursorStyleDataUrl(id: CursorStyleId, state: CursorStyleState = "rest"): string {
	const key = `${id}:${state}`;
