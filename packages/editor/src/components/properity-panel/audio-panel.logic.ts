...
export function isPresetActive(
	...
	return (
		Math.abs(settings.fadeIn - preset.in) < 0.01 && Math.abs(settings.fadeOut - preset.out) < 0.01
	);
}

/** Label of the matching preset (drives the Segmented selection); "" = custom. */
export function activePreset(settings: { fadeIn: number; fadeOut: number }): string {
 ...
 */
export function envelopePath(fadeIn: number, fadeOut: number, clipDuration: number): string {
	const W = 100;
	const H = 24;
