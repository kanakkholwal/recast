...
// A grid of real previews rather than a dropdown: each tile renders through CaptionBox, so it shows what the export burns in.
...
// One spoken and one unspoken word: the gap between them is what the progressive highlight actually looks like.
...
// One size for every tile: scaling by each theme's `fontSizePct` made small themes unreadable and display faces overflow.
