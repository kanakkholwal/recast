...
export class FrameCache {
	...
	#stats = { entryCount: 0, bytes: 0, oldestEntryUs: -1 };
	#storage: FrameStorage;
	...
	replaceStorage(storage: FrameStorage): void {
		...
		this.#memory.clear();
		this.#stats = { entryCount: 0, bytes: 0, oldestEntryUs: -1 };
	 ...
	 * decode.
	 */
	readMemory(tsUs: number): ImageBitmap | VideoFrame | null {
	 ...
	 */
	write(tsUs: number, frame: CacheableFrame, persist = true): void {
		...
		if (persist) {
			...
			void this.#storage
				.put(tsUs, frame, performance.now() * 1000)
				.catch((err) => {
	...
	#memoryInsert(tsUs: number, frame: ImageBitmap | VideoFrame): void {
		const size = estimateFrameBytes(frame as CacheableFrame);
