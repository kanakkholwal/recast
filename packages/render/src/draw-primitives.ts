/**
 * Headless Canvas2D draw primitives for the annotation/scene renderer: arrow
 * geometry, rounded-rect path, stroke dashes, blur-variant tint, alpha helper.
 * Pure (no app/DOM deps) and typed for both `CanvasRenderingContext2D` (preview)
 * and `OffscreenCanvasRenderingContext2D` (export), so ONE renderer feeds both.
 */

/** The path-drawing surface shared by the on-screen and offscreen contexts. */
export type Ctx2D = CanvasRenderingContext2D | OffscreenCanvasRenderingContext2D;

export type StrokeStyle = "solid" | "dashed" | "dotted";

/** Canvas dash array for a stroke style, scaled by stroke width. Empty = solid. */
export function strokeDashPattern(style: StrokeStyle | undefined, strokePx: number): number[] {
 ...
 * opacity, mirroring the export exactly (`base_alpha = 0.15 + 0.80*strength`,
 * `× opacity`, glass grey wash past 0.6). `color` variant parses a `#rrggbb`
 * (with/without `#`); invalid colour → null.
 */
 ...
 * shadow colour so the cast glow dims without fading the shape itself.
 */
 ...
 * rounds cleanly regardless of dimensions.
 */
