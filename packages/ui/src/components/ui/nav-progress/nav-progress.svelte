 ...
 *
 * `active` is the router's in-flight flag, passed in by the host: this
 * package stays framework-agnostic, so SvelteKit's `navigating` is read in
 * the app (`<NavProgress active={navigating.to !== null} />`) rather than
 * imported here. On start: jumps to `minimum`, then trickles up toward 0.9
 * while pending — never 1.0 until the navigation completes. On finish:
 * snaps to 1.0, fades out.
 *
