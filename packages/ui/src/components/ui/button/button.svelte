...
export const buttonVariants = tv({
	...
	variants: {
		...
		size: {
			default:
				"h-9 rounded-lg px-5 py-2.5 text-sm font-medium gap-2 [&_svg:not([class*='size-'])]:size-4",
			xl: "h-12.5 rounded-xl py-3.5 px-7 text-lg font-medium gap-2.5 [&_svg:not([class*='size-'])]:size-6",
			lg: "h-11 rounded-xl px-8 text-base font-semibold gap-2 [&_svg:not([class*='size-'])]:size-5",
